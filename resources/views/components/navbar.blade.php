<nav class="fixed inset-x-0 top-0 z-50 px-3 py-3 sm:px-6 sm:py-4 lg:px-10">
    <div
        data-public-nav
        class="mx-auto flex max-w-[1500px] items-center justify-between rounded-2xl border border-white/10 bg-[#061A35]/30 px-3.5 py-2.5 backdrop-blur-xl sm:px-6 sm:py-3"
    >
        <a href="/" class="group flex min-w-0 items-center gap-2 sm:gap-3">
            <span class="flex h-10 w-10 shrink-0 items-center justify-center transition duration-300 group-hover:scale-105 sm:h-12 sm:w-12">
                <img
                    src="{{ asset('images/branding/sozo-habitat-emblem-dark.svg') }}"
                    alt="Emblème Sozo Habitat"
                    class="h-10 w-10 object-contain drop-shadow-[0_6px_16px_rgba(0,0,0,0.22)] sm:h-12 sm:w-12"
                >
            </span>

            <span class="leading-none">
                <span class="block truncate text-base font-black tracking-tight text-white sm:text-xl">
        ...
        <details class="relative md:hidden">
            <summary class="flex h-10 w-10 cursor-pointer list-none items-center justify-center rounded-xl border border-white/20 bg-white/10 text-white transition hover:bg-white/20 sm:h-11 sm:w-11 [&::-webkit-details-marker]:hidden">
                <svg viewBox="0 0 24 24" fill="none" class="h-6 w-6" stroke="currentColor" stroke-width="2" aria-hidden="true">
                    <path stroke-linecap="round" d="M4 7h16M4 12h16M4 17h16"/>
                </svg>
            </summary>

            <div class="absolute right-0 mt-3 w-[min(16rem,calc(100vw-1.5rem))] overflow-hidden rounded-2xl border border-slate-200 bg-white p-2.5 text-[#0A2E5D] shadow-2xl sm:p-3">
